 ...
 */
class IsolatedMarginTransferHistoryQuery extends Query
{
    use PrivateQuery, DatePagedQuery, QueryParamValidator;

    // Endpoint properties
    protected string $method = 'GET';
    protected string $resource = '/sapi/v1/margin/isolated/transfer';
    protected int $weight = 1;

    // Request properties
    protected array $required = ['symbol', 'endTime'];

    public const TYPE_SPOT = 'SPOT';
    public const TYPE_ISOLATED_MARGIN = 'ISOLATED_MARGIN';

    private static array $types = [
        self::TYPE_SPOT,
        self::TYPE_ISOLATED_MARGIN,
    ];
    ...
    public function setTransFrom(string $transFrom): self
    {
        static::assertParamInArray($transFrom, static::$types, 'Transfer from');

        $this->transFrom = $transFrom;
        return $this;
    }

    public function setTransTo(string $transTo): self
    {
        static::assertParamInArray($transTo, static::$types, 'Transfer to');
    ...
    {
        static::assertInBetween($size, 2, 100, 'Size');
        ...
        foreach ($this->pagedExecute() as $data) { // Date based pagination
            do {
                ...
                foreach ($data['rows'] as $row) {
                    $row['date'] = (new \DateTime())->setTimestamp(intval($row['timestamp'] / 1000))->format('Y-m-d H:i:s');
                    $results[] = $row;
                }
